...
async function generateAISignal() {
  ...
  try {
    ...
    setLoadingState(true, 'Running signal analysis...');

    // ── STEP 5: Rule-based engine — 3/5 strategies must agree ──
    const signal = runRuleBasedEngine(candles, indicators, structure, livePrice, pip, selectedPair, tf, account, riskpct);

    // ── STEP 6: Render ──
    renderAISignal(signal, { livePrice, account, riskpct, pip, tf, indicators, structure });
...
// ─────────────────────────────────────
// STEP 4 — RULE-BASED SIGNAL ENGINE
// 5 strategies, need 3/5 to fire signal
// ─────────────────────────────────────

function runRuleBasedEngine(candles, indicators, structure, livePrice, pip, pair, tf, account, riskpct) {
  const ind = indicators;
  const str = structure;
  const precision = pip < 0.001 ? 5 : pip < 0.01 ? 4 : pip < 1 ? 2 : 2;
  const fmt = v => parseFloat(v).toFixed(precision);
  const atr = ind.atr || pip * 15;

  // ── 5 STRATEGY VOTES ──
  // Each returns: { vote: 'BUY'|'SELL'|null, name, reason }

  // Strategy 1: EMA Trend Alignment
  // Price > EMA20 > EMA50 > EMA200 = strong bull, opposite = strong bear
  function stratEMATrend() {
    const bullAlign = ind.aboveEma20 && ind.aboveEma50 && ind.aboveEma200
                   && ind.ema20 > ind.ema50 && ind.ema50 > ind.ema200;
    const bearAlign = !ind.aboveEma20 && !ind.aboveEma50 && !ind.aboveEma200
                   && ind.ema20 < ind.ema50 && ind.ema50 < ind.ema200;
    if (bullAlign) return { vote:'BUY',  name:'EMA Trend Alignment', reason:`Price above all EMAs in bull order (20>50>200). Strong uptrend confirmed.` };
    if (bearAlign) return { vote:'SELL', name:'EMA Trend Alignment', reason:`Price below all EMAs in bear order (20<50<200). Strong downtrend confirmed.` };
    return { vote:null, name:'EMA Trend Alignment', reason:'EMAs not aligned — no clear trend.' };
  }

  // Strategy 2: RSI + Order Block Confluence
  // RSI oversold at bullish OB = buy, RSI overbought at bearish OB = sell
  function stratRSIOrderBlock() {
    const rsi = ind.rsi;
    if (rsi < 35 && str.nearBullOB) return {
      vote:'BUY', name:'RSI + Order Block',
      reason:`RSI ${rsi.toFixed(1)} oversold at Bullish OB [${fmt(str.nearBullOB.low)}–${fmt(str.nearBullOB.high)}]. Institutional demand zone with oversold momentum.`
    };
    if (rsi > 65 && str.nearBearOB) return {
      vote:'SELL', name:'RSI + Order Block',
      reason:`RSI ${rsi.toFixed(1)} overbought at Bearish OB [${fmt(str.nearBearOB.low)}–${fmt(str.nearBearOB.high)}]. Institutional supply zone with overbought momentum.`
    };
    // Softer version: RSI extremes alone
    if (rsi < 30) return { vote:'BUY',  name:'RSI + Order Block', reason:`RSI ${rsi.toFixed(1)} in oversold zone. Reversal momentum building.` };
    if (rsi > 70) return { vote:'SELL', name:'RSI + Order Block', reason:`RSI ${rsi.toFixed(1)} in overbought zone. Reversal momentum building.` };
    return { vote:null, name:'RSI + Order Block', reason:`RSI ${rsi.toFixed(1)} neutral — no extreme reading.` };
  }

  // Strategy 3: MACD Cross + Structure Break
  // Fresh MACD crossover in direction of market structure
  function stratMACDStructure() {
    const bullBOS = str.hhhl; // Higher highs / higher lows
    const bearBOS = str.lhll; // Lower highs / lower lows
    if (ind.macdCross && bullBOS) return {
      vote:'BUY', name:'MACD + Structure Break',
      reason:`Bullish MACD crossover confirmed with ${str.structureBias}. Momentum aligned with trend.`
    };
    if (ind.macdBearCross && bearBOS) return {
      vote:'SELL', name:'MACD + Structure Break',
      reason:`Bearish MACD crossover confirmed with ${str.structureBias}. Momentum aligned with trend.`
    };
    // MACD cross without structure — weaker signal
    if (ind.macdCross && !bearBOS) return { vote:'BUY',  name:'MACD + Structure Break', reason:`Bullish MACD crossover. Histogram flipped positive.` };
    if (ind.macdBearCross && !bullBOS) return { vote:'SELL', name:'MACD + Structure Break', reason:`Bearish MACD crossover. Histogram flipped negative.` };
    return { vote:null, name:'MACD + Structure Break', reason:'No fresh MACD crossover detected.' };
  }

  // Strategy 4: FVG Fill Setup
  // Price inside a Fair Value Gap = magnet for price to fill
  function stratFVGFill() {
    if (str.nearBullFVG) {
      const midFVG = (str.nearBullFVG.top + str.nearBullFVG.bottom) / 2;
      const aboveMid = livePrice > midFVG;
      return {
        vote:'BUY', name:'FVG Fill Setup',
        reason:`Price inside Bullish FVG [${fmt(str.nearBullFVG.bottom)}–${fmt(str.nearBullFVG.top)}]. Imbalance acts as support, expecting continuation up.`
      };
    }
    if (str.nearBearFVG) {
      return {
        vote:'SELL', name:'FVG Fill Setup',
        reason:`Price inside Bearish FVG [${fmt(str.nearBearFVG.bottom)}–${fmt(str.nearBearFVG.top)}]. Imbalance acts as resistance, expecting continuation down.`
      };
    }
    return { vote:null, name:'FVG Fill Setup', reason:'Price not inside any detected Fair Value Gap.' };
  }

  // Strategy 5: Liquidity Sweep Reversal
  // Price swept a swing high/low and closed back — classic stop hunt reversal
  function stratLiquiditySweep() {
    if (str.sweptLow) return {
      vote:'BUY', name:'Liquidity Sweep Reversal',
      reason:`Bullish liquidity sweep below ${fmt(str.prevSwingLow)}. Smart money hunted stops, expecting sharp reversal upward.`
    };
    if (str.sweptHigh) return {
      vote:'SELL', name:'Liquidity Sweep Reversal',
      reason:`Bearish liquidity sweep above ${fmt(str.prevSwingHigh)}. Smart money hunted stops, expecting sharp reversal downward.`
    };
    // Secondary: EMA50 bounce in trend direction
    const bounceUp   = ind.aboveEma200 && livePrice > ind.ema50 * 0.9995 && livePrice < ind.ema50 * 1.003;
    const bounceDown = !ind.aboveEma200 && livePrice < ind.ema50 * 1.0005 && livePrice > ind.ema50 * 0.997;
    if (bounceUp)   return { vote:'BUY',  name:'EMA50 Bounce', reason:`Price bouncing off EMA50 in bullish trend. High-probability continuation long.` };
    if (bounceDown) return { vote:'SELL', name:'EMA50 Bounce', reason:`Price bouncing off EMA50 in bearish trend. High-probability continuation short.` };
    return { vote:null, name:'Liquidity Sweep Reversal', reason:'No liquidity sweep or EMA bounce detected.' };
  }

  // ── TALLY VOTES ──
  const strategies = [stratEMATrend(), stratRSIOrderBlock(), stratMACDStructure(), stratFVGFill(), stratLiquiditySweep()];
  const buyVotes   = strategies.filter(s => s.vote === 'BUY');
  const sellVotes  = strategies.filter(s => s.vote === 'SELL');
  const triggered  = buyVotes.length >= sellVotes.length ? buyVotes : sellVotes;
  const direction  = buyVotes.length > sellVotes.length ? 'BUY' : sellVotes.length > buyVotes.length ? 'SELL' : null;

  // Need at least 3/5 strategies to agree
  if (!direction || triggered.length < 3) {
    return {
      verdict:    'WAIT',
      confidence: Math.max(buyVotes.length, sellVotes.length) * 20,
      strategies,
      buyCount:   buyVotes.length,
      sellCount:  sellVotes.length,
      reasoning:  `Only ${triggered.length}/5 strategies agree (need 3). Market conditions unclear — no high-probability setup detected. Wait for clearer confluence.`,
      wait_reason: `${buyVotes.length} BUY vs ${sellVotes.length} SELL signals. Missing: ${strategies.filter(s=>!s.vote).map(s=>s.name).join(', ')}.`,
    };
  }
  ...
  // Use ATR as minimum distance
  let sl, tp1, tp2;

  if (isBuy) {
    const swingLow = str.swingLows.length > 0 ? Math.min(...str.swingLows) : entryPrice - atr * 2;
    sl  = Math.min(swingLow, entryPrice - atr * 1.5);
    sl  = sl - atr * 0.3; // buffer below swing
    tp1 = entryPrice + (entryPrice - sl) * 1.5;
    tp2 = entryPrice + (entryPrice - sl) * 2.5;
  } else {
    const swingHigh = str.swingHighs.length > 0 ? Math.max(...str.swingHighs) : entryPrice + atr * 2;
    sl  = Math.max(swingHigh, entryPrice + atr * 1.5);
    sl  = sl + atr * 0.3; // buffer above swing
    tp1 = entryPrice - (sl - entryPrice) * 1.5;
    tp2 = entryPrice - (sl - entryPrice) * 2.5;
  }

  const slPips = Math.abs(entryPrice - sl) / pip;
  const rrr    = '1:' + ((Math.abs(tp2 - entryPrice) / Math.abs(entryPrice - sl))).toFixed(1);
  const confidence = triggered.length === 5 ? 92 : triggered.length === 4 ? 78 : 64;

  // ── BUILD REASONING ──
  const triggeredNames = triggered.map(s => s.name).join(', ');
  const reasoning = `${triggered.length}/5 strategies confirmed ${direction}.

` +
    triggered.map(s => `✓ ${s.name}: ${s.reason}`).join('

') +
    `

Structure: ${str.structureBias} | RSI: ${ind.rsi?.toFixed(1)} | ATR: ${fmt(atr)}`;
  ...
  return {
    ...
    tp2:        fmt(tp2),
    rrr,
    sl_pips:    slPips.toFixed(0),
    entry_type: `Market ${direction} — ${triggered.length}/5 strategies confirmed`,
    sl_note:    isBuy ? `Below swing low + ${(atr/pip*0.3).toFixed(0)}pip buffer` : `Above swing high + ${(atr/pip*0.3).toFixed(0)}pip buffer`,
    tp1_note:   `1:1.5 RR — partial profit`,
    tp2_note:   `1:2.5 RR — full target`,
    frameworks: triggered.map(s => s.name),
    reasoning,
    management: `Move SL to breakeven when TP1 is hit. Let remaining position run to TP2. ${isBuy ? 'Trail stop below each new higher low.' : 'Trail stop above each new lower high.'}`,
    key_risk:   `${strategies.filter(s => !s.vote || s.vote !== direction).map(s => s.name).join(', ')} did not confirm — watch for reversal.`,
  };
}

// ─────────────────────────────────────
// STEP 4 — BUILD AUTONOMOUS PROMPT (kept for reference)
// ─────────────────────────────────────
...
function renderAISignal(sig, meta) {
  ...
  // Show triggered strategies as badges
  const trigCount = sig.triggered ? sig.triggered.length : 0;
  const totalStrats = 5;
  const indBadges = [
    `<span class="strat-badge ${trigCount >= 3 ? 'trend' : 'scalp'}">${trigCount}/${totalStrats} Strategies</span>`,
    ...
    `<span class="strat-badge ${ind.macdBullish ? 'trend' : 'scalp'}">RSI ${ind.rsi?.toFixed(0)}</span>`,
  ].join('');
...
function setLoadingState(on, msg) {
  ...
  if (btn) { btn.disabled = on; btn.textContent = on ? '⏳  Analysing...' : '📊  Analyse'; }
  const out = document.getElementById('output');
