...
async function seedMetalsCrypto() {
  // Try CoinGecko free API for real crypto prices (no key needed)
  try {
    const res  = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,litecoin,ripple&vs_currencies=usd&include_24hr_change=true');
    const data = await res.json();
    const map  = { bitcoin:'BTCUSD', ethereum:'ETHUSD', litecoin:'LTCUSD', ripple:'XRPUSD' };
    Object.entries(map).forEach(([id, sym]) => {
      if (data[id]) {
        const price = data[id].usd;
        const chg   = +(data[id].usd_24h_change || 0).toFixed(2);
        liveData[sym] = {
          price, open: price / (1 + chg/100),
          high: +(price * 1.003), low: +(price * 0.997),
          change: chg, changePct: chg,
          dir: chg >= 0 ? 'up' : 'dn', source: 'coingecko',
        };
      }
    });
  } catch(e) {
  ...
  // Metals — use FCS proxy if available, else last known reasonable estimates
  const metalSeeds = {
    XAUUSD: { base:2387, vol:0.003 },
    XAGUSD: { base:28.5, vol:0.006 },
    XPTUSD: { base:980,  vol:0.005 },
    XPDUSD: { base:1020, vol:0.005 },
  };
  // Only seed metals that haven't been set by FCS proxy
  Object.entries(metalSeeds).forEach(([sym, cfg]) => {
    if (!liveData[sym] || liveData[sym].source === 'loading') {
      ...
      liveData[sym] = {
        ...
        high: +(price * 1.003), low: +(price * 0.997),
        change:chg, changePct:chg,
        dir: chg >= 0 ? 'up' : 'dn', source:'est',
      };
    }
  });

  // Crypto fallback seeds if CoinGecko failed
  const cryptoSeeds = {
    BTCUSD: { base:83500, vol:0.01 },
    ETHUSD: { base:1920,  vol:0.012 },
    LTCUSD: { base:88,    vol:0.015 },
    XRPUSD: { base:2.1,   vol:0.016 },
  };
  Object.entries(cryptoSeeds).forEach(([sym, cfg]) => {
    if (!liveData[sym] || liveData[sym].source === 'loading') {
      const drift = cfg.base * (Math.random() * cfg.vol * 2 - cfg.vol);
      const price = +(cfg.base + drift);
      const chg   = +(drift / cfg.base * 100).toFixed(2);
...
function buildIndices() {
  const seeds = {
    US30:   { base:41800, vol:0.003 },
    NAS100: { base:18200, vol:0.004 },
    SP500:  { base:5580,  vol:0.003 },
    GER40:  { base:22100, vol:0.004 },
    UK100:  { base:8600,  vol:0.003 },
    JPN225: { base:35800, vol:0.004 },
  };
